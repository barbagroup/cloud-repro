"""Plot the spanwise-averaged vorticity field at different time steps."""
...
name = 'wz-avg'  # name of the field variable

# Get directories.
rootdir = pathlib.Path(__file__).absolute().parents[1]
datadir = rootdir / 'output' / 'postprocessing' / 'wz_avg'
...
filepath = rootdir / 'snake3d35.body'
with open(filepath, 'r') as infile:
    xb, yb, _ = numpy.loadtxt(infile, skiprows=1, unpack=True)
...
fig, ax = pyplot.subplots(nrows=len(times), figsize=(5.0, 5.0), sharex=True)
if len(times) == 1:
    ax = [ax]
