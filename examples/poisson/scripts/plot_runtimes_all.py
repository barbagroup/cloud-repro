...
data = {'Colonial One': {}, 'Colonial One (OpenMPI)': {}, 'Azure': {}}
...
# Get runtimes data for PETSc runs on Colonial One (OpenMPI).
nodes = [1, 2, 4, 8]
subdata = {'nodes': [], 'min': [], 'max': [], 'means': []}
for n in nodes:
    casedir = rootdir / f'colonialone-openmpi/petsc/{n:0>2}_short/output'
    filepaths = get_filepaths(casedir)
    runtimes = amgxwrapper_poisson_read_runtimes(*filepaths)
    store_runtime_stats(subdata, n, get_runtime_stats(runtimes))
data['Colonial One (OpenMPI)']['PETSc'] = gather_arrays(subdata)
# Get runtimes data for AmgX runs on Colonial One (OpenMPI).
nodes = [1, 2, 4, 8]
subdata = {'nodes': [], 'min': [], 'max': [], 'means': []}
for n in nodes:
    casedir = rootdir / f'colonialone-openmpi/amgx/{n:0>2}_ivygpu/output'
    filepaths = get_filepaths(casedir)
    runtimes = amgxwrapper_poisson_read_runtimes(*filepaths)
    nites = amgxwrapper_poisson_read_iterations(filepaths[0])
    store_runtime_stats(subdata, n, get_runtime_stats(runtimes,
                                                      scale=1.0 / nites))
data['Colonial One (OpenMPI)']['AmgX'] = gather_arrays(subdata)
...
# Colonial One (OpenMPI) runs.
nodes, mins, maxs, means = data['Colonial One (OpenMPI)']['PETSc']
ax1.plot(numpy.log2(nodes), means, label='Colonial One (OpenMPI)', color='C2')
ax1.errorbar(numpy.log2(nodes), means,
             [means - mins, maxs - means],
             fmt='k', linewidth=0, ecolor='black', elinewidth=2,
             capthick=2, capsize=4, barsabove=True)
...
ax2.errorbar(numpy.log2(nodes), means,
             ...
             fmt='k', linewidth=0, ecolor='black', elinewidth=2,
             capthick=2, capsize=4, barsabove=True)
nodes, mins, maxs, means = data['Colonial One (OpenMPI)']['AmgX']
ax2.plot(numpy.log2(nodes), means, label='Colonial One (OpenMPI)', color='C2')
ax2.errorbar(numpy.log2(nodes), means,
             [means - mins, maxs - means],
             fmt='k', linewidth=0, ecolor='black', elinewidth=2,
             capthick=2, capsize=4, barsabove=True)
...
filepath = figdir / 'poisson_time_vs_nodes_all.pdf'
fig.tight_layout()
