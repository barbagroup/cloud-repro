...
For small message sizes, the averaged latency on Colonial One and Azure are $1.75$ and $1.95 \mu s$, respectively.
...
For all message sizes, a higher bandwidth rates was achieved on Azure.
...
For example, the projection method implemented in our software solve a Poisson system every time step to project the velocity field onto the divergence-free space (to satisfy the incompressibility constraint).
Thus, we investigated the time-to-solution to solve a three-dimensional Poisson system (obtained with a 7-point stencil central-difference scheme) on different nodes of Microsoft Azure and Colonial One.
The system was solved using a Conjugate-Gradient (CG) method with a classical Algebraic Multi-Grid (AMG) preconditioner with an exit criterion set to an absolute tolerance of $10^{-12}$.
...
The Poisson system for a base mesh of $6.25$ million cells ($500 \times 500 \times 25$) is solved on a single node using $12$ MPI processes and $2$ GPU devices; we then double the mesh size as we double the number of nodes (keeping the same number of MPI processes and GPUs per node).
...
The smaller runtimes on Azure are explained by the fact the NC-series of Microsoft Azure features NVIDIA K80 GPU devices with a higher compute capability than the K20 GPUs on Colonial One.
...
We observe larger variations in the time-to-solution for the Poisson system on Microsoft Azure; runtimes are more constant on Colonial One.
...
One of the requirements to produce reproducible computational results is the make the code available.
...
The task completed $200,000$ time steps (i.e., $80$ time units of flow simulation with time-step size $\Delta t = 0.0004 c / U_\infty$) in just above $7$ hours.
...
Although the two-dimensional simulations gave us some insights into to flow dynamics happening in the behind the snake, we know that at this Reynolds number ($Re = 2000$), three-dimensional structures will develop in the wake.
Thus, we submitted jobs to Azure Batch to perform direct numerical simulation of the three-dimensional flow around snake model: a cylinder with the same anatomically accurate cross-section.
...
With a ``pay-as-you-go'' subscription, users can immediately sees how much it costs to run a scientific application on a public cloud; such information is often hidden to end-users on university-managed clusters.
...
Command-line utilities such as Azure-CLI and Batch Shipyard were greatly helpful to create and manage resources on Microsoft Azure to conduct reproducible research.
...
Azure-CLI helped us setup the Azure Batch and Storage accounts as well as moving data between the cloud platform and our local machines.
Thanks to Batch Shipyard, we did not have to dig into the software development kit of Azure Batch to use the service.
Writing YAML configuration files was all we needed to create a pool of virtual machines with Batch Shipyard and to submit jobs to it.
...
Note that Bach Shipyard also support Singularity containers.

Our in-house CFD software, PetIBM, relies on MPI to run applications on distributed-memory architectures.
The Poisson system is solved on distribute GPU devices using the NVIDIA AmgX library.
Thus, we have used Azure instances of the NC-series that feature a network interface for remote direct memory access (RDMA) connectivity, allowing nodes in the same pool to communicate over InfiniBand network.
As of this writing, only Intel MPI 5.x versions are supported with the Azure Linux RDMA drivers.

Moreover, Microsoft Azure implements quotas and limits on resources with Azure Batch service, such as the maximum number of dedicated cores that can used in a certain region.
...
In a new effort to make our research transparent and reproducible by others, we have version-controlled all the files necessary to re-run the examples highlighted in this paper.
The repository \texttt{cloud-repro}\footnote{\url{https://github.com/barbagroup/cloud-repro}} is hosted on GitHub.
The reproducibility packages contains input files, Batch Shipyard configuration files, command-line instructions to setup and submit jobs to Microsoft Azure, as well as the post-processing scripts to reproduce the figures of this paper.
...
These files contain details about the all libraries installed to create the computational runtime environment.
In addition, we have also uploaded to Zenodo all output data required to just reproduce the figures of the present paper.

\section*{Acknowledgments}

This study was possible thanks to a sponsorship from the Microsoft Azure for Research program\footnote{Microsoft Azure for Research: \url{https://www.microsoft.com/en-us/research/academic-program/microsoft-azure-for-research/}} and supported by NSF Grant No. CCF-1747669.
