...
The grid used for the three-dimensional simulation contains about $46$ million cells and is somewhat coarse for Direct Numerical Simulation (DNS) at Reynolds number $2,000$, but acceptable for our exploratory analysis.
A finer grid would be preferable to capture the flow dynamics in details, but the purpose of this paper is not targeting the physics.
...
We decided to continue this work with the coarser grid due to the cost difference (see Section \ref{sec:cost}).
...
Indeed, tools in our workflow that we have no control over (Azure CLI, Batch Shipyard) will likely change, and perhaps not be backwards compatible. 
That is inevitable. 
...
We are also immensely grateful to the reviewers, Patrick O'Leary \cite{oleary_2019} and Freddie Witherden \cite{witherden_2019}, for their suggestions.
