...
Reproducible research and replication studies are essential components for the progress of evidence-based science, even more since science today advances via computation.
More than ever we use computer simulations to create new knowledge.
But how do we certify that this new knowledge is justified, that there is enough evidence to support it?
With constant increase in the rate of submissions to journals as well as software and data getting more complex, it becomes difficult to verify and corroborate what computational scientists present in journal and conferences \cite{donoho_et_al_2009}.
...
We follow this convention, clarified more recently in a report from the National Academies of Sciences, Engineering, and Medicine.
...
We note the presence of a recirculation region behind the snake cylinder and the deformation of the von Karman street as the spanwise vortices are convected away from the body (Fig. \ref{fig:ux_uy_xz_plane_3d}).
Fig. \ref{fig:qcrit_wx_3d} shows the isosurfaces of the Q-criterion after $100$ time units and highlights the complexity of the three-dimensional wake generated by the snake model.
...
\begin{figure*}[!h]
    ...
    \caption{Lateral view of the isosurfaces of the Q-criterion ($Q = 1.0$) in the wake of the snake cylinder (with a $35$-degree angle of attack) at Reynolds number $2000$. The isosurfaces are colored with the streamwise vorticity ($-5 \leq w_x c / U_\infty \leq 5$).}
...
In a new effort to make our research transparent and reproducible by others, we have version-controlled all the input files, pre- and post-processing scripts, as well as the Batch Shipyard configuration files required to rerun the simulations on Microsoft Azure.
