...
Command-line tools such as Azure-CLI and Batch Shipyard were greatly helpful to create and submit everything from our local terminal. (In our research lab, we tend to avoid, as much as possible, interacting with graphical-user interfaces, to make our workflow more reproducible.)
Azure-CLI helped us to setup the Azure Batch account and its associated Azure Storage, as well as uploading/downloading simulation data.
With Batch Shipyard, we did not need to have experience with the software development kit to use Azure Batch.
We have used Batch Shipyard to submit jobs on Azure Batch to run Docker container-based multi-instance tasks.
