...
For example, the projection Navier-Stokes solver implemented in our software requires the solution of a Poisson system at every time step to project the velocity field onto the divergence-free space (to satisfy the incompressibility constraint).
...
Microsoft Azure offers the possibility to take advantage of surplus capacity with ``low-priority'' virtual machines that are significantly cheaper than ``dedicated'' ones (\ref{tab:nc_series}).
For example, the low-priority \texttt{NC24r} instance has an hourly cost of $0.792$ USD, $5$ times cheaper than its dedicated counterpart.
However, the reader should keep in mind that low-priority may not be available for allocation or may be preempted at any time.
In fact, low-priority instances should be avoided for long-running MPI jobs in pools where inter-node communication is enabled.
We only used dedicated virtual machines to run our CFD software.
Moreover, our job tasks used a shared filesystems (GlusterFS on compute nodes) for I/O operations and Batch Shipyard will fail to create the pool if low-priority nodes are requested.
...
Latency and bandwidth micro-benchmarks show that our university-managed HPC system and Microsoft Azure can produce comparable runtimes.
...
We plan to analyze the output data from these runs to be published in a later paper.
