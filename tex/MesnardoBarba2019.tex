...
(The output of those simulations are now being processed to further analyze the complex flow dynamics generated behind the snake model.)
...
\IEEEPARstart{L}{ooking} for an alternative solution to university-managed HPC clusters to run CFD simulations with our in-house software, we leveraged the public cloud provider Microsoft Azure to generate reproducible computational results.
Our reproducible workflow makes use of Docker containers to faithfully port the runtime environment our CFD application to Azure.
We used the command-line utilities Azure-CLI and Batch Shipyard to run multi-instance tasks within Docker containers on the Azure Batch platform.
Our CFD simulations ran on instances of the NC-series that feature NVDIDA K80 GPU devices and enable access to InfiniBand network.
The benchmarks showed that we could get comparable runtimes between our university-managed HPC system and Microsoft Azure.
Thanks to Microsoft Azure sponsorship, we have been able to run dozens of CFD simulations of a gliding snake's model.
The study of the three-dimensional flow around a cylinder with an anatomically accurate cross-section of the flying snake is currently ongoing; output data from these runs are being analyzed and will be published in a later paper.
